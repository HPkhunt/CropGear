...
async def upload_review_image(
    ...
    raise APIException(
        "Legacy review uploads are no longer supported. Upload via /media/presign and /media/finalize using purpose=review_photo.",
...
async def submit_review(
    ...
    if role != "admin":
        ...
        if status_value != "completed":
            raise APIException(
                "Only completed bookings are eligible for review.", status.HTTP_400_BAD_REQUEST
            )
        end_date = _parse_date(booking.get("end_date"))
        if end_date and end_date > _today_utc():
            raise APIException(
                "Review can be submitted after the booking end date.", status.HTTP_400_BAD_REQUEST
            )
    ...
    if not payload.title.strip() and not payload.comment.strip():
        raise APIException(
            "Provide a title or comment for the review.", status.HTTP_400_BAD_REQUEST
        )
...
async def respond_to_review(
    ...
    if review.get("review_type") != "equipment":
        raise APIException(
            "Responses are only supported for equipment reviews.", status.HTTP_400_BAD_REQUEST
        )
    if role != "admin" and review.get("recipient_id") != reviewer_id:
        raise APIException(
            "You can only respond to reviews for your listings.", status.HTTP_403_FORBIDDEN
        )
    if review.get("status") != "approved":
        raise APIException(
            "Review must be approved before responding.", status.HTTP_400_BAD_REQUEST
        )
...
async def dispute_review(
    ...
    if review.get("recipient_id") != user_id and current_user.get("role") != "admin":
        raise APIException(
            "Only the review recipient can open a dispute.", status.HTTP_403_FORBIDDEN
        )
...
async def moderation_queue(
    status_filter: str = Query(
        default="pending", pattern=r"^(pending|flagged|disputed|rejected|approved|hidden|all)$"
    ),
    page: int = Query(default=1, ge=1),
    page_size: int = Query(default=20, ge=1, le=100),
    current_user: dict = Depends(get_current_user),
...
async def owner_review_analytics(
    ...
    responded_count = await db.reviews.count_documents(
        {**match, "response.message": {"$exists": True}}
    )
    ...
    recent_docs = await db.reviews.find(match).sort("created_at", -1).limit(5).to_list(length=5)
