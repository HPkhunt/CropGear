...
class MediaPresignRequest(BaseModel):
    ...
    expires_in: Optional[int] = Field(
        default=DEFAULT_PRESIGN_EXPIRES_IN, ge=60, le=MAX_PRESIGN_EXPIRES_IN
    )
...
async def media_capabilities():
    readiness = await media_service.get_upload_readiness()
    uploads_enabled = bool(readiness.get("ready"))
    return {
        "s3_configured": uploads_enabled,
        "s3_ready": uploads_enabled,
        "s3_bucket_name": readiness.get("bucket") or None,
        "equipment_image_upload_enabled": uploads_enabled,
        "review_photo_upload_enabled": uploads_enabled,
        "chat_attachment_upload_enabled": uploads_enabled,
...
async def presign_media_upload(
    ...
    readiness = await media_service.ensure_upload_ready()
    bucket = str(readiness.get("bucket") or "").strip()

    key = media_service.build_key(purpose, owner_id, payload.filename, payload.content_type)
    now = _now_iso()
    expires_in = min(
        max(int(payload.expires_in or DEFAULT_PRESIGN_EXPIRES_IN), 60), MAX_PRESIGN_EXPIRES_IN
    )
...
async def get_media_asset(
    ...
    if (
        role != "admin"
        and not asset.get("is_public")
        and asset.get("owner_id") != current_user.get("sub")
    ):
    ...
    if asset.get("status") == "ready":
        download_url = await media_service.create_presigned_get(
            asset.get("s3_key", ""), int(expires_in)
        )
