...
class EmailService:
    ...
    async def send_booking_status_update(
        ...
        *,
        ...
        end_date = booking.get("end_date", "N/A")
        total_amount = self._format_currency(booking.get("total_amount", 0))
        status_label = str(status_value or "updated").replace("_", " ").title()
        intro_line = intro or f"Your booking is now marked as {status_label.lower()}."
        subject = f"{self._brand()} Booking {status_label}"
        body = (
            f"{intro_line}\n\n"
            f"Booking ID: {booking_id}\n"
            f"Equipment: {equipment_name}\n"
        ...
        html = (
            f"<h2>{self._brand()} Booking {status_label}</h2>"
            f"<p>{intro_line}</p>"
            f"<ul>"
            f"<li><strong>Booking ID:</strong> {booking_id}</li>"
            f"<li><strong>Equipment:</strong> {equipment_name}</li>"
    ...
    async def send_payment_receipt(
        self, booking: dict, recipient_email: str, amount: float, currency: str = "USD"
    ):
        equipment_name = booking.get("equipment_name", "your equipment")
        booking_id = booking.get("id", "N/A")
