...
class EquipmentRepository(BaseRepository):
    ...
    async def _owner_verification_map(self, owner_ids: Iterable[str]) -> Dict[str, bool]:
        ...
        for doc in docs:
            verified_map[str(doc["_id"])] = bool(doc.get("is_verified", False)) and bool(
                doc.get("is_active", True)
            ) and doc.get("approval_status", "approved") == "approved"
        return verified_map

    async def _annotate_owner_verification(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def browse(
        ...
        filters: Dict[str, Any] = {
            "daily_rate": {"$gte": float(min_rate), "$lte": float(max_rate)}
        }
