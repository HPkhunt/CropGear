...
class MockDatabase:
    ...
    def _seed(self) -> None:
        ...
        email_profile_map = {
            ...
            "owner2@cropgear.com": {"full_name": "Liam TractorWorks", "phone_number": "+1-555-010-2222"},
            "owner3@cropgear.com": {"full_name": "Sophia HarvestTech", "phone_number": "+1-555-010-3333"},
            "owner4@cropgear.com": {"full_name": "Ethan FieldMachines", "phone_number": "+1-555-010-4444"},
        ...
        for index, demo_user in enumerate(demo_users, start=1):
            profile = email_profile_map.get(
                ...
                default_profiles.get(demo_user["role"], {"full_name": "Demo User", "phone_number": ""}),
            )
            self._users.append(
                {
                    "id": f"u{index}",
        ...
        while len(self._equipment) < target_count:
            ...
            owner = owners[(len(self._equipment) - 1) % max(len(owners), 1)] if owners else {"id": "u2", "full_name": "Noah Equipment"}
        ...
        default_owner = owners[0] if owners else {"id": "u2", "full_name": "Noah Equipment"}
        self._bookings = []
    ...
    def _persist_runtime_equipment(self) -> None:
        runtime_items = [item for item in self._equipment if item.get("id") not in self._seed_equipment_ids]
        try:
    ...
    def authenticate(self, credential: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        user = self.get_user_by_email(credential) if "@" in credential else self.get_user_by_phone(credential)
    ...
    def _is_owner_verified(self, owner_id: str) -> bool:
        ...
        if not owner:
            return False
        return bool(owner.get("is_verified", False)) and owner.get("approval_status", "approved") == "approved"
    ...
    def create_equipment(self, owner_id: str, owner_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def set_equipment_visibility(self, equipment_id: str, visible: bool) -> Optional[Dict[str, Any]]:
    ...
    def delete_equipment(self, equipment_id: str) -> bool:
        index = next((i for i, item in enumerate(self._equipment) if item.get("id") == equipment_id), None)
        if index is None:
            return False
    ...
    def owner_requests(self, owner_id: Optional[str] = None) -> List[Dict[str, Any]]:
        if owner_id is None:
            ...
            items = [b for b in self._bookings if b["owner_id"] == owner_id and b["booking_status"] == "pending"]
        return deepcopy(items)

    def update_booking_status(self, booking_id: str, status: str, actor_id: str, actor_role: str) -> Dict[str, Any]:
    ...
    def create_booking(self, farmer_id: str, start_date: str, end_date: str, equipment_id: str) -> Dict[str, Any]:
    ...
    def dashboard_metrics(self) -> Dict[str, Any]:
        pending_owners = len([u for u in self._users if u["role"] == "equipment_owner" and not u["is_verified"]])
        pending_users = len([u for u in self._users if u.get("approval_status", "approved") == "pending"])
        
        confirmed_bookings = [b for b in self._bookings if b.get("booking_status") in {"confirmed", "completed"}]
    ...
    def list_users_for_approval(self, status_filter: str = "pending", role_filter: str = "all") -> List[Dict[str, Any]]:
        users = self._users
        if role_filter != "all":
    ...
    def list_owner_verifications(self, status_filter: str = "all") -> List[Dict[str, Any]]:
        owners = [u for u in self._users if u["role"] == "equipment_owner"]
        if status_filter == "pending":
            owners = [u for u in owners if not u.get("is_verified", False)]
        elif status_filter == "verified":
            owners = [u for u in owners if u.get("is_verified", False)]
        elif status_filter == "rejected":
            owners = [u for u in owners if not u.get("is_active", True)]
    ...
    def set_owner_verification_status(self, owner_id: str, status: str) -> Optional[Dict[str, Any]]:
        for user in self._users:
            if user["id"] != owner_id or user.get("role") != "equipment_owner":
                continue
    ...
    def revenue_summary(self, period: str = "30d") -> Dict[str, Any]:
        ...
        confirmed = [b for b in bookings_in_window if b.get("booking_status") in {"confirmed", "completed"}]
        revenue = sum(float(b.get("total_amount", 0)) for b in confirmed)
        admin_revenue = sum(float(b.get("admin_cut", 0)) for b in confirmed)
        utilization = round((len(confirmed) / max(len(bookings_in_window), 1)) * 100, 1) if bookings_in_window else 0.0
        ...
        return {
            ...
            "average_rating": round(sum(e["rating"] for e in self._equipment) / max(len(self._equipment), 1), 2),
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "owners": max(len([u for u in self._users if u.get("role") == "equipment_owner"]), base["owners"]),
