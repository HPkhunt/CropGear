...
def _profile_for_email(email: str, role: str) -> Dict[str, str]:
    ...
    return email_profile_map.get(email.lower(), default_profiles.get(role, {"full_name": "Demo User", "phone_number": ""}))
...
def _generate_equipment_seed(owners: List[Dict[str, Any]], target_count: int = 60) -> List[Dict[str, Any]]:
    created = _now()
    ...
    try:
        ...
        if testimonials_count == 0:
            ...
            await db.testimonials.insert_many(
                [
                    {"quote": "Excellent service, highly recommend!", "author": "A. Farmer", "created_at": now},
                    {"quote": "Found the perfect trailer quickly.", "author": "B. Owner", "created_at": now},
                ]
            )
